...
function serveStatic (root, options) {
  ...
  return function serveStatic (req, res, next) {
    ...
    if(pathname.slice(-5)==".html"){
      (function(req, pathname, opts, res){


        var full_path = path.join(opts.root, pathname);

        console.log(pathname, opts, req.url, full_path);

        fs.readFile(full_path, "utf-8", function(err, content){
          if(!err){
            render(content, function(err, htmlStr){
              if(!err){
                res.statusCode = 200;
